...
class CustomUser(AbstractUser):
    ...
    bot_secret = models.CharField(max_length=100, blank=True, null=True)
    ...
    def generate_referral_code():
        ...
        while True:
            code = 'CX' + ''.join(random.choices(chars, k=6))
...
class Deposit(models.Model):
    ...
    bitcoin_txid = models.CharField(max_length=100, blank=True, null=True)
    ...
    def send_status_notification(self, old_status, new_status):
        try:
            ...
            logger.error(f"Failed to send deposit status email: {e}")
...
class ReferralReward(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk and not self.is_paid:
            try:
                ...
                logger.error(f"Failed to credit referral bonus: {e}")
        super().save(*args, **kwargs)
